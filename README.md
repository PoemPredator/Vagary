# 填词作品统计工具（以V大Vagary为例，项目文件基本由AI生成）
...
如果你偏好命令行或想在脚本/自动化里运行，可以参考下面的最小命令集合（此部分为可选补充，项目仍以 Notebook 操作为主，初学者建议先按上面的 Notebook 指南）：
...
- 强制不使用 jieba（使用内置保底分词）：

```
python -m vagary --no-jieba
```

命令行选项的说明见代码（`python -m vagary --help`），常用参数包括 `--data/-d`、`--output/-o`、`--user-dict`、`--exclude/-e`、`--search/-s`、`--no-jieba`。
